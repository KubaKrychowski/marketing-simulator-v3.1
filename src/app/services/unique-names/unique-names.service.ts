...
export class UniqueNamesService {
  ...
  checkIfCompanyNameIsAvailable(companyName: string) {
    return this.http.post<String>(
      `${API_URL}/BusyNames/companyName/${companyName}`,
      null,
      { headers: new HttpHeaders().set('token', this.apiService.clientToken) }
    );
  }

  checkIfProductNameIsAvailable(productName: string) {
    return this.http.post<String>(
      `${API_URL}/BusyNames/productName/${productName}`,
      null,
      { headers: new HttpHeaders().set('token', this.apiService.clientToken) }
    );
  }
